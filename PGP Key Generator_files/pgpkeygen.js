...
var genKeyPair = function () {
	...
	$('#generate_keys_btn').css('background-color', '#333');
	$('#generate_keys_btn').val("Working .");
	 	...
	 	if (!err) {
			...
			alice.sign({}, function(err) {
				...
				alice.export_pgp_private_to_client ({
					...
					$('#download_priv_key').css('pointer-events', 'auto');
					$('#download_priv_key').css('background', '#FF2424');
		      	});
			  	alice.export_pgp_public({}, function(err, pgp_public) {
		        	_debug("public key: " + pgp_public);
					$('#pubkey').val(pgp_public);
					// Enable download buttons
					$('#download_pub_key').css('pointer-events', 'auto');
					$('#download_pub_key').css('background', '#FF2424');
		      	});
		    });
	  	}
	  	
	  	// Enable button once again (NOTE: user should refresh to re-gen)
	  	//$('#generate_keys_btn').css('pointer-events', 'auto');
	  	//$('#generate_keys_btn').css('background-color', '#FF2424');
